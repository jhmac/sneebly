...
class CircuitBreaker {
  ...
  checkState(): void {
    if (this.isOpen) {
      throw new Error(
        `Circuit breaker open: ${this.failures} consecutive failures detected`
      );
    }
  }
...
export interface BatchOptions {
  ...
  maxTimeout?: number;
  /** Overall operation timeout in ms (default: 300000 = 5 minutes) */
  timeout?: number;
...
export async function batchProcess<T, R>(
  ...
  const timeoutPromise = new Promise<never>((_, reject) =>
    setTimeout(() => reject(new Error(`Batch operation timed out after ${timeout}ms`)), timeout)
  );

  return Promise.race([Promise.all(promises), timeoutPromise]) as Promise<R[]>;
}
