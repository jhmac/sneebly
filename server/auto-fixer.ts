...
Allowed commands: npx drizzle-kit push/generate/migrate/check, npx tsc --noEmit, npm run <script> (NOT dev/start), npm test, npx eslint, cat, ls, head, tail, grep, find, wc, mkdir, cp, mv, touch, sed, awk, diff, sort, uniq, git status/diff/log. NEVER run npm install or npm run dev/start (causes port conflicts).
...
async function processBlockers(): Promise<FixResult[]> {
  ...
  try {
    ...
    if (shouldStopForBudget().stop) {
      console.log("[AutoFixer] Budget exceeded (hard stop) — skipping");
      isRunning = false;
      return results;
    }
